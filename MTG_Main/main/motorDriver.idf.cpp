...
void MotorDriver::move(float x, float y)
{
    ...
    else
    {
        ...
        ret = rmt_transmit(motor_channel_b, &constant_curve->base, &tx_config, 1, &tx_config);
        if (ret != 0)
        {
    ...
    rmt_tx_wait_all_done(motor_channel_a, 30000);
    rmt_tx_wait_all_done(motor_channel_b, 30000);
}
